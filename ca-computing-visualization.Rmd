...
To the right is a visualization of all the computing courses in high schools throughout California. These courses were identified based on their content. 
...
m <- leaflet() %>%
  ...
  addCircles(data = dplyr::filter(courses,coursecode == "2451"),~lng, ~lat, 
    popup = paste(popup_info), weight = 6, radius = 60,
  ...
  addCircles(data = dplyr::filter(courses,coursecode == "2453"),~lng, ~lat, 
    popup = paste(popup_info), weight = 3, radius = 40,
  ...
  addCircles(data = dplyr::filter(courses,coursecode == "2465"),~lng, ~lat, 
    popup = popup_info, weight = 3, radius = 40,
...
datatable(dplyr::select(courses,districtcode, schoolcode, countyname,
  ...
  caption = "Data source:www.cde.gov", 
  rownames = FALSE,
